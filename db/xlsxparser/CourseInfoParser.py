...
def cache_and_flatten_xlsx(file_path):
    ...
    # 병합 셀 평탄화: 각 셀에 대해 병합 셀의 좌측 상단이 아닌 경우 건너뛰고,
    # 이후 collapse_consecutive_nones 함수로 None 연속 축소 수행
    flattened_data = []
...
def parse_xlsx_final(file_path):
    ...
    파일 내 캐싱된 데이터에서 "1. 교과목 정보"라는 문자열을 포함한 행 이후부터 시작하여,
    "2. 교과목 개요"라는 문자열이 나타나기 전까지 각 행을 파싱하여
    결과 딕셔너리 (예: { "개설연도": "2020년", "학기": "1학기", ... })에 합친다.
    """
    ...
    for row in cached_rows:
        ...
        if any(cell is not None and "1. 교과목 정보" in str(cell) for cell in row):
            ...
            continue
        if in_section:
            # "2. 교과목 개요" (또는 구분자로 사용할 문자열)이 나타나면 섹션 종료
