// survey_simplified.js - 간소화된 5단계 설문조사
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateButtonVisibility() {
        ...
        nextBtn.textContent = currentStep === steps.length - 1 ? "제출" : "다음";
    }
    ...
    function validateCurrentStep() {
        switch (currentStep) {
            case 0: // 1단계: 학점 입력
                ...
                const total = parseInt(majorCredit) + parseInt(electiveCredit);
                if (total > 24) {
                    alert("총 학점은 24학점을 초과할 수 없습니다.");
                    return false;
                }
                break;
            ...
            case 1: // 2단계: 공강 요일 선택
                ...
                if (checkedDays.length === 0) {
                    alert("희망하는 공강 요일을 선택해주세요. (없으면 '없음'을 선택)");
                    return false;
                }
                break;
    ...
    function submitSurvey() {
        ...
        const freeDays = Array.from(document.querySelectorAll(".weekday-options input:checked"))
            .map(cb => cb.value)
            .filter(val => val !== "없음");
        ...
        console.log("설문조사 결과 (5단계 간소화 버전):", surveyData);
    ...
    function startTimetableGeneration(surveyData) {
        ...
        const convertedConstraints = {
            ...
            preference_tags: surveyData.preferenceTags || []
        };
        ...
        setTimeout(() => {
            ...
            // 시간표 생성 완료 후 피드백 메시지
            setTimeout(() => {
                showSurveyProgressMessage('시간표 생성이 완료되었습니다!', false);
            }, 3000);
        }, 500);
    }

    // 진행 메시지 표시 함수
    function showSurveyProgressMessage(message, showLoading) {
        ...
        if (overlay) {
            ...
            overlay.style.display = showLoading ? 'block' : 'none';
        }
    }
